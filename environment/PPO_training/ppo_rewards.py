...
def phase2_reward(obs, action, next_obs, absorbing, env, ep=None):
    ...
    # I phase2_reward og phase3_reward
    if np.linalg.norm(puck_vel) < 0.01:
        reward -= 2.0  # Straff for stillestående puck (kan justeres)
    ...
    if -0.5 < mallet_x < 0.5 and mallet_y < 0:  # Midten av egen halvdel
        reward += 1.5  # Belønning for forsvarsposisjon
...
def phase3_reward(obs, action, next_obs, absorbing, env, ep=None):
    ...
    # I phase2_reward og phase3_reward
    if np.linalg.norm(puck_vel) < 0.01:
        reward -= 1.0  # Straff for stillestående puck (kan justeres)
