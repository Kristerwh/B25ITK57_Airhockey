...
from environment.PPO_training.rewards.exp_old_rewards.ppo_rewards_phase2 import phased_reward as reward_fn
...
def main(render=True):
    ...
    plot_dir = f"ppo_training_plots/phase2_{run_id}"
    os.makedirs(plot_dir, exist_ok=True)
    writer = SummaryWriter(f"runs/ppo_airhockey_phase2_{run_id}")
        ...
        with mujoco.viewer.launch_passive(model, data) as viewer:
            for episode in trange(total_episodes):
                ...
                if (episode + 1) % 50 == 0:
                    ...
                    trainer.save("PPO_training_saved_models/phase2/saved_model")
    else:
        for episode in trange(total_episodes):
            run_episode(episode)
            if (episode + 1) % 50 == 0:
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"ppo_training_plots/phase2_{run_id}/ppo_training_reward_curve.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"ppo_training_plots/phase2_{run_id}/ppo_training_diagnostics.png")
    plt.close()
