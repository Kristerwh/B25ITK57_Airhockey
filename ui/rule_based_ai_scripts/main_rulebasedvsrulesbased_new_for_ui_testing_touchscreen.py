...
with mujoco.viewer.launch_passive(model, data, show_left_ui=False, show_right_ui=False) as viewer:
    ...
    while viewer.is_running():
        ...
        #print(actionsend)
        action = script.run(scripted_ai1, puck_pos, mallet_pos_script_ai)
        action2 = script.run(scripted_ai2, puck_pos_reverted, mallet2_pos)
        action2 = np.array([-action2[0], -action2[1]])
        combined = np.concatenate((action[:2], action2[:2]))
        next_obs, reward, absorbing, _ = env.step(combined)
